...
function prepareSearchIndex(pdfs) {
    ...
    pdfs.forEach(pdf => {
        // Handle Firestore Timestamps and strings
        let uploadDateObj;
...
function renderPDFs() {
    ...
    const filteredPdfs = pdfDatabase.filter(pdf => {
        // Early return for cheap class and semester checks
        ...
        const matchesSearch = searchTerm ? pdf._searchStr.includes(searchTerm) : true;
...
function createPDFCard(pdf, favoritesList, index = 0, highlightRegex = null) {
    ...
    const formattedDate = pdf._formattedDate || new Date().toLocaleDateString('en-US'); // Fallback just in case
