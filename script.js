...
function prepareSearchIndex(pdfs) {
    const now = new Date();
    pdfs.forEach(pdf => {
        const titleStr = typeof pdf.title === 'string' ? pdf.title : '';
        const descStr = typeof pdf.description === 'string' ? pdf.description : '';
        const catStr = typeof pdf.category === 'string' ? pdf.category : '';
        const authorStr = typeof pdf.author === 'string' ? pdf.author : '';

        // Add pre-calculated derived fields for faster searching
...
function renderPDFs() {
    ...
    const filteredPdfs = pdfDatabase.filter(pdf => {
        ...
        // Optimize search using pre-calculated _searchStr
