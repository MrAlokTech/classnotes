...
function prepareSearchIndex(data) {
    ...
    data.forEach(pdf => {
        ...
        // Handle Firestore Timestamp or ISO String gracefully
        let uploadDateObj;
...
async function loadPDFDatabase() {
    ...
    try {
        ...
        if (shouldUseCache) {
            ...
            // PRE-CALCULATE search strings & dates BEFORE rendering
        ...
        // --- FIX: CALL THIS TO POPULATE UI ---
        syncClassSwitcher();
        renderSemesterTabs(); // Ensure category filters and tabs also stay synced
        renderCategoryFilters();
        renderPDFs();
        hidePreloader();
...
function renderPDFs() {
    ...
    const filteredPdfs = pdfDatabase.filter(pdf => {
        // Early returns for cheap equality checks to save CPU cycles
        ...
        // Use pre-calculated search string instead of multiple toLowerCase() calls
        // Fallback to old behavior if _searchStr isn't available for some reason
        if (pdf._searchStr) {
...
function createPDFCard(pdf, favoritesList, index = 0, highlightRegex = null) {
    ...
    // Use pre-calculated values if available, otherwise fallback to computing on the fly
    const isNew = pdf._isNew !== undefined ? pdf._isNew : (() => {
        const uploadDateObj = new Date(pdf.uploadDate);
        return (new Date() - uploadDateObj) < (7 * 24 * 60 * 60 * 1000); // 7 days
    })();
    ...
    // Use pre-calculated formatted date if available
