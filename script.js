...
function prepareSearchIndex(data) {
    const now = new Date();
    // Cache the formatter for maximum performance
    ...
    data.forEach(pdf => {
        ...
        pdf._searchStr = `${title} ${description} ${category} ${author}`.toLowerCase();

        // Handle both Firestore Timestamp (.toDate) and string dates
        let uploadDateObj;
        if (pdf.uploadDate && typeof pdf.uploadDate.toDate === 'function') {
            ...
            uploadDateObj = now; // Fallback
        }

        // Pre-calculate expensive date logic
