...
/* =========================================
   ...
   ========================================= */
// ⚡ Bolt: Added prepareSearchIndex to pre-calculate expensive properties.
// This moves the string concatenation (for search), Date parsing, and date formatting
// out of the render loop and into the initialization phase, significantly improving render speed.
function prepareSearchIndex() {
    ...
    pdfDatabase.forEach(pdf => {
        ...
        if (!pdf._formattedDate || pdf._isNew === undefined) {
...
function renderPDFs() {
    ...
    // ⚡ Bolt: Implemented early returns in filter logic to short-circuit evaluation.
    // Also using pre-calculated _searchStr for faster keyword matching.
    const filteredPdfs = pdfDatabase.filter(pdf => {
        ...
        if (searchTerm && pdf._searchStr) {
            if (!pdf._searchStr.includes(searchTerm)) {
                return false;
            }
        }
...
function createPDFCard(pdf, favoritesList, index = 0, highlightRegex = null) {
    ...
    // ⚡ Bolt: Using pre-calculated date properties to avoid Date parsing in the render loop.
