...
function prepareSearchIndex() {
    ...
    for (let i = 0; i < pdfDatabase.length; i++) {
        ...
        // Handle Firestore Timestamp or standard string
        let uploadDateObj;
...
async function loadPDFDatabase() {
    ...
    try {
        ...
        if (shouldUseCache) {
            ...
            prepareSearchIndex(); // ⚡ Bolt Optimization: Pre-calculate derived search strings and dates
...
function renderPDFs() {
    ...
    // ⚡ Bolt Optimization: Use early returns and pre-calculated _searchStr for faster filtering
...
function createPDFCard(pdf, favoritesList, index = 0, highlightRegex = null) {
    ...
    // ⚡ Bolt Optimization: Use pre-calculated values instead of creating Date objects on every render
