...
/**
 * BOLT OPTIMIZATION: Pre-calculate derived fields to avoid
 * expensive Date parsing and string operations during high-frequency
 * render/filter loops.
 */
function prepareSearchIndex(data) {
    const now = new Date();
    data.forEach(pdf => {
        // 1. Pre-calculate search string (lowercased)
        pdf._searchStr = `${pdf.title || ''} ${pdf.description || ''} ${pdf.category || ''} ${pdf.author || ''}`.toLowerCase();

        // 2. Pre-calculate date properties
        // Handle Firestore Timestamps or regular strings
        let uploadDateObj;
...
async function loadPDFDatabase() {
    ...
    try {
        ...
        if (shouldUseCache) {
            ...
            prepareSearchIndex(pdfDatabase); // BOLT OPTIMIZATION: Index cached data
        ...
        // BOLT: Cache BEFORE indexing to save localStorage space
        localStorage.setItem(CACHE_KEY, JSON.stringify({
            timestamp: new Date().getTime(),
            data: pdfDatabase
        }));

        prepareSearchIndex(pdfDatabase); // BOLT OPTIMIZATION: Index fresh data
...
function renderPDFs() {
    ...
    const filteredPdfs = pdfDatabase.filter(pdf => {
        // BOLT OPTIMIZATION: Early returns for fast checks
...
function createPDFCard(pdf, favoritesList, index = 0, highlightRegex = null) {
    ...
    // Formatting Date - BOLT OPTIMIZATION: Use pre-calculated value
